...
| v1.8 | 2026-06-30 00:00:00 | Add release-stamp cache-busting for app.js/styles.css; fix redundant IndexedDB read in loadPendingQueueCache | Claude | perf_cache_bust_and_pending_queue_read |
...
- 手機優先，避免需要橫向捲動。
- `index.html` 引用 `app.js`/`styles.css` 必須帶 `?v=<stamp>` cache-busting query string，避免 GitHub Pages/瀏覽器快取舊版本。本專案無 build step，`<script src="app.js?v=...">` 的 query string 數值與 `app.js` 內的 `APP_RELEASE_STAMP` 是兩處手動維護的值，**每次更新 `APP_RELEASE_STAMP` 時必須同步更新 `index.html` 裡的 query string**，否則 cache-busting 不會生效。
...
- `未選`、`未送`、`安康耕莘`、`亞東` 可轉為 `hospital='其他'` + `hospital_custom=<原值>`，用於相容舊 DB。
- Production DB 已套用 v1.2 後可接受 `安康耕莘`，但前端仍保持向下相容。
- `亞東` 採與 `安康耕莘` 相同模式：前端表單可選，但 DB 端先以 `其他` + `hospital_custom` 儲存，尚未新增對應 Supabase check constraint migration。
...
- `loadPendingQueueCache()` 每個 owner（user id）只需從 IndexedDB 實際讀取一次；之後即使 queue 清空為 0 筆，也不應重複觸發 IndexedDB 讀取，需用「是否已載入過」的旗標區分，而不是用陣列長度判斷。
